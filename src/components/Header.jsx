...
export default function Header() {
  ...
  return (
    <>
      ...
      <div className="h-16 sm:h-20 w-full hidden md:block"></div>

      <header className="fixed top-0 left-0 right-0 z-[1000] w-full transition-all duration-500 ease-in-out">
        {/* Top Info Strip (Fades out smoothly on scroll for compact sticky header) */}
        <div 
          className={`bg-[#07111e] text-slate-300 text-xs py-1.5 px-4 border-b border-slate-800/60 hidden md:block transition-all duration-300 overflow-hidden ${
            isScrolled ? 'max-h-0 py-0 opacity-0 pointer-events-none' : 'max-h-12 opacity-100'
          }`}
        >
        ...
        <nav 
          className={`w-full transition-all duration-400 ease-in-out ${
            isScrolled 
              ? 'bg-[#0b1a2e]/95 backdrop-blur-xl text-white shadow-[0_8px_32px_rgba(0,0,0,0.4)] py-2.5 border-b border-[#facc15]/20' 
              : 'bg-[#0b1a2e] text-white py-3.5 shadow-md'
          }`}
        >
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between">
            
            {/* Logo & Icon with Pure White Container */}
            <NavLink to="/" className="flex items-center gap-3 group focus:outline-none">
              <div className="w-11 h-11 sm:w-13 sm:h-13 rounded-2xl bg-white p-1 shadow-[0_4px_20px_rgba(250,204,21,0.3)] border-2 border-[#facc15] flex items-center justify-center overflow-hidden transition-transform group-hover:scale-105">
                <img 
                  src="/logo.jpg" 
              ...
              <div className="flex flex-col">
                <div className="flex items-baseline gap-1.5">
                  <span className="text-lg sm:text-2xl font-black font-cairo text-white">
                    توب رايز
                  </span>
                  <span className="text-base sm:text-xl font-extrabold font-sans text-[#facc15]">
                    TOP RISE
                  </span>
                </div>
                <span className="text-[10px] text-slate-300 font-semibold leading-tight hidden sm:block">
            ...
            <div className="hidden lg:flex items-center gap-1.5">
              {navigation.map((item) => (
                <NavLink
                  ...
                  to={item.path}
                  className={({ isActive }) =>
                    `px-4 py-2 rounded-full text-sm font-bold transition-all relative ${
                      isActive
                        ? 'text-[#facc15] bg-[#facc15]/15 after:content-[""] after:absolute after:bottom-0 after:left-1/2 after:-translate-x-1/2 after:w-1/2 after:h-0.5 after:bg-[#facc15] after:rounded-full'
                        : 'text-slate-200 hover:text-[#facc15] hover:bg-[#facc15]/10'
                    }`
                  }
                >
